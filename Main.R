...
df$Period <- df$Year - 2017 + df$Month / 12

p1 <- ggplot(data = df, aes(x = Period, y = Voters, colour = Type)) +
  geom_point() +
  geom_line() +
  scale_y_continuous(limits = c(0, max(df$Voters))) +
...
print(paste("Upper regression line coefs:", coefs_upper))
print(paste("Base regression line coefs:", coefs_base))
print(paste("Lower regression line coefs:", coefs_lower))
correlation_coef <- cor(df2$Voters, df2$Period)
print(paste("Correlation Coefficient:", correlation_coef))
print(paste("Correlation Coefficient:", correlation_coef ^ 2))
...
p3 <- ggplot(monthly, aes(x = Month, y = Proportion, group = factor(Year))) +
  ...
  scale_y_continuous(limits = c(0, max(monthly$Proportion))) +
  labs(title = "Monthly Turnout Proportions of Each Year", y = "Voter Turnout %")

p4 <- ggplot(monthly, aes(x = Month, y = Proportion)) +
  geom_boxplot(fill = "pink") +
  geom_jitter(width = 0.2, alpha = 0.2) +
  geom_hline(yintercept = 1/12, linetype = "dashed", color = "orange") +
  scale_y_continuous(limits = c(0, max(monthly$Proportion))) +
  labs(title = paste0("Distributions of Monthly Turnout % (2017-", max(monthly$Year), ")"), y = "Turnout % of Each Year")

df2 <- monthly[monthly$Year > 2018,]
print(t.test(df2[df2$Month == "Jan",]$Proportion, mu = 1/12, alternative = "less"))

p4a1 <- ggplot(df2, aes(x = Month, y = Proportion)) +
  geom_boxplot() +
  geom_jitter(width = 0.2, alpha = 0.2) +
  geom_hline(yintercept = 1/12, linetype = "dashed", color = "orange") +
  stat_summary(fun = mean, geom = "point", shape = 20, size = 3, color = "orange") +
  scale_y_continuous(limits = c(0, max(df2$Proportion))) +
...
df2[df2$Month == "Jan",]$Proportion
ggsave("Monthly Turnout Percent Dist.png", p4, width = 7, path = "plots")
ggsave("MTPD Excluding Underestimates.png", p4a1, width = 7, path = "plots")
